...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.controller.title('OSHWAL ATM')
        self.controller.state('normal')
        self.controller.iconphoto(False,tk.PhotoImage(file='abc.png'))

        heading=tk.Label(self,text='OSHWAL ATM',font=('orbitron',45,'bold'),foreground='black',background='lightblue')
        heading.pack(pady=25)

        space_label=tk.Label(self,height=4,bg='black').pack()

        password_label=tk.Label(self,text=(f'Welcome {user_display_name} to OSHWAL COLLEGE ATM'),font=('BatmanForeverAlternate',17),bg='black',fg='cyan').pack(pady=10)

        def next_page():
            controller.show_frame("MenuPage")

        entry_button = tk.Button(self,text='Enter2',font=('orbitron',12),command=next_page,relief='raised',borderwidth=3,width=23,height=3).pack(pady=10)
        ...
        dualtone_label=tk.Label(self, text='',font=('orbitron',13),fg='white',bg='black',anchor='n')
        ...
        #credits=tk.Label(bottom_frame,text='Created and Develped TAlib',font=('orbitron',15)).pack()
...
class MenuPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        heading=tk.Label(self,text='Oshwal  atm',font=('orbitron',45,'bold'),foreground='black',background='lightblue')
        heading.pack(pady=25)
        main_menu_label=tk.Label(self,text='Main Menu',font=('orbitron',13),fg='black',bg='lighblue')
        main_menu_label.pack(pady=5)
        slection_label=tk.Label(self,text='Please make a selection',font=('orbitron',13),fg='white',bg='black')
        ...
        withdraw_button=tk.Button(button_frame,text='Withdraw',font=('orbitron',13),command=withdraw,relief='raised',borderwidth=3,width=20,height=4)
        withdraw_button.grid(row=0,column=0,pady=7)

        def deposit():
            controller.show_frame('DepositPage')

        deposit_button=tk.Button(button_frame,text='Deposit',font=('orbitron',13),command=deposit,relief='raised',borderwidth=3,width=20,height=4)
        deposit_button.grid(row=1,column=0,pady=5)
        def balance():
            controller.show_frame('BalancePage')

        balance_button=tk.Button(button_frame,text='Balance',font=('orbitron',13),command=balance,relief='raised',borderwidth=3,width=20,height=4)
        balance_button.grid(row=0,column=1,pady=7,padx=794)

        def info():
            controller.show_frame('InfoPage')

        info_button=tk.Button(button_frame,text='Personal Info',font=('orbitron',13),command=info,relief='raised',borderwidth=3,width=20,height=4)
...
class WithdrawPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        heading=tk.Label(self,text='OSHWAL ATM',font=('orbitron',45,'bold'),foreground='white',background='black')
        heading.pack(pady=25)
        choose_amount_label=tk.Label(self,text='Choose the amount you want to withdraw',font=('orbitron',13),fg='cyan',bg='white')
        ...
        def withdraw(amount):
            ...
            if amount>current_balance:
                ...
                mydb=sqlite3.connect('oshwal.db')
                mycursor=mydb.cursor()
                mycursor.execute(f"update CUSTOMERS2 set balance ={current_balance} where accid = {username1} ")
                mydb.commit()
        ...
        def deposit_cash():
            ...
            try:
                ...
                if int(val)<0:
                    ...
                    controller.show_frame('MenuPage')
                    cash.set('')
                    mydb=sqlite3.connect.connect(host="localhost",user="root",password=123456)
                    mycursor=mydb.cursor()
...
class InfoPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        mydb=sqlite3.connect('oshwal.db')
        mycursor=mydb.cursor()
        mycursor.execute(f"select password from CUSTOMERS2 where accid = {username1} ")
...
def register_user():
  ...
  mycursor.execute('select accid from CUSTOMERS2')
  ...
  if username_info in b:
    ...
    name_entry.delete(0,END)
        ...
        name_entry.delete(0, END)
        screen1.destroy()
        mycursor.execute("insert into CUSTOMERS2 values('"+username_info+"','"+name_info+"','"+password_info+"','"+balance_inti+"')")
        mydb.commit()
      ...
      if username1.isdigit():
          ...
          if int(username1) in user_acc:
              ...
              if password1=='':
                  ...
                  mycursor.execute(f"select name from CUSTOMERS2 where accid={username1}")
                  values=mycursor.fetchall()
                  user_name=[]
                  for i in values:
                  ...
                  mycursor.execute(f'select current_balance from CUSTOMERS2 where accid ={username1}')
                  values=mycursor.fetchall()
                  user_balance=[]
                  for i in values:
...
def main_screen():
  ...
  screen.title("oshwal atm")
  screen.configure(bg='lightblue')
  screen.iconphoto(False,tk.PhotoImage(file='abc.png'))


  Label(text = "OSHWAL",fg='white', bg = "black", width = "300", height = "2", font = ("orbitron", 15,'bold')).pack()
